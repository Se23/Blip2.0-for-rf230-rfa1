...
//      printf("Forwarding, header_off = %i ",header_off);
	...
	printf("------------------ WARNING: Found header_off = %i ------------------\n",header_off);
        //return;

/*LOG-Zitat:

rcvd [TOS IPRaw.recv Forwarding, header_off = 1136 
------------------ WARNING: Found header_off = 1136 ------------------
Do SEND]
rcvd [TOS IPRaw.recv Forwarding, header_off = 1136 
------------------ WARNING: Found header_off = 1136 ------------------
Do SEND]

Vor dem Auskommentieren wären diese empfangenen IP-(Ping)-Pakete verworfen worden. Durch die Warnung, wurden die "fehlerhaften" Pakete sichtbar, wurden aber dennoch erfolgreich übertragen.
*/
      } 
      if(1){ //<---else{
      ...
      if (!(signal ForwardingEvents.approve[next_hop_ifindex](&pkt, next_hop))){
        printf("------------------ WARNING: Packet not approved ------------------\n");
	return;
	}
